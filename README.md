...
AI-агент для аналитики данных JIRA через естественный язык. Задаёте вопрос на русском — получаете данные из базы.
...
└── closed-env-package/         # Пакет для закрытого контура (JupyterLab, без интернета)
    ├── 01_data_loader.py       # Автогенерация моделей + JIRA Knowledge Base (22 таблицы)
    ├── 02_build_faiss.py       # Построение FAISS-индекса
...
Data loader содержит встроенную базу знаний (22 таблицы JIRA) с:
- Русскими описаниями таблиц и колонок
- Рекомендуемыми мерами (open_count, resolved_count, link_count, status_change_count)
- Подсказками по колонкам (pkey, summary, assignee, reporter)

KB работает как страховочная сеть: дополняет описания GigaChat при ошибках генерации.
